...
})({"7nUAM":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4e5dac8afe405db7";
module.bundle.HMR_BUNDLE_ID = "d7335db75fb5518a";
"use strict";
...
(function(win, doc, undefined) {
    RGraph.SVG.Bar = function(conf) {
        ...
        this.layers = {
        }; // MUST be before the SVG tag is created!
        ...
        this.draw = function() {
            ...
            for(var i = 0; i < this.data_seq.length; ++i){
                if (typeof properties.errorbars[i] === 'undefined' || RGraph.SVG.isNull(properties.errorbars[i])) properties.errorbars[i] = {
                    max: null,
                    min: null
                };
                else if (typeof properties.errorbars[i] === 'number') properties.errorbars[i] = {
                    min: properties.errorbars[i],
                    max: properties.errorbars[i]
                };
                else if (typeof properties.errorbars[i] === 'object' && typeof properties.errorbars[i].max === 'undefined') properties.errorbars[i].max = null;
                else if (typeof properties.errorbars[i] === 'object' && typeof properties.errorbars[i].min === 'undefined') properties.errorbars[i].min = null;
            }
            //
            ...
            for(var i = 0, max = 0; i < this.data.length; ++i){
                ...
                if (properties.errorbars && typeof properties.errorbars[i] === 'number') var errorbar = properties.errorbars[i];
                else if (properties.errorbars && typeof properties.errorbars[i] === 'object' && typeof properties.errorbars[i].max === 'number') var errorbar = properties.errorbars[i].max;
                else var errorbar = 0;
                if (typeof this.data[i] === 'number') values.push(this.data[i] + errorbar);
            ...
            if (properties.xaxisLabels && properties.xaxisLabels.length) {
                ...
                // Label substitution
                //
                for(var i = 0; i < properties.xaxisLabels.length; ++i)properties.xaxisLabels[i] = RGraph.SVG.labelSubstitution({
                    object: this,
        ...
        this.drawBars = function() {
            ...
            for(var i = 0, sequentialIndex = 0; i < this.data.length; ++i, ++sequentialIndex){
                ...
                if (typeof this.data[i] === 'number') {
                    var outerSegment = (this.graphWidth - properties.marginInnerLeft - properties.marginInnerRight) / this.data.length, height = (Math.abs(this.data[i]) - Math.abs(this.scale.min)) / (Math.abs(this.scale.max) - Math.abs(this.scale.min)) * this.graphHeight, width = (this.graphWidth - properties.marginInnerLeft - properties.marginInnerRight) / this.data.length - properties.marginInner - properties.marginInner, x = properties.marginLeft + properties.marginInner + properties.marginInnerLeft + outerSegment * i;
                    ...
                    if (this.scale.min >= 0 && this.scale.max > 0) y = this.getYCoord(this.scale.min) - height;
                        ...
                        if (this.data[i] < 0) y = this.getYCoord(0);
                    } else if (this.scale.min < 0 && this.scale.max < 0) {
                ...
                //
                    ...
                    for(var j = 0; j < this.data[i].length; ++j, ++sequentialIndex){
                        var width = (innerSegment - (this.data[i].length - 1) * properties.marginInnerGrouped) / this.data[i].length, x = outerSegment * i + properties.marginInner + properties.marginLeft + properties.marginInnerLeft + j * width + (j - 1) * properties.marginInnerGrouped;
                        ...
                        // eg 0 -> 10
                        if (this.scale.min === 0 && this.scale.max > this.scale.min) var height = (this.data[i][j] - this.scale.min) / (this.scale.max - this.scale.min) * this.graphHeight, y = this.getYCoord(0) - height;
                        else if (this.scale.max <= 0 && this.scale.min < this.scale.max) {
                            var height = (this.data[i][j] - this.scale.max) / (this.scale.max - this.scale.min) * this.graphHeight, y = this.getYCoord(this.scale.max);
                            height = Math.abs(height);
                        // eg 10 -> -10
                        } else if (this.scale.max > 0 && this.scale.min < 0) var height = Math.abs(this.data[i][j]) / (this.scale.max - this.scale.min) * this.graphHeight, y = this.data[i][j] < 0 ? this.getYCoord(0) : this.getYCoord(this.data[i][j]);
                        else if (this.scale.min > 0 && this.scale.max > this.scale.min) var height = Math.abs(this.data[i][j] - this.scale.min) / (this.scale.max - this.scale.min) * this.graphHeight, y = this.getYCoord(this.scale.min) - height;
                        // Add the rect tag
                        var rect = RGraph.SVG.create({
                            svg: this.svg,
                        ...
                        if (!this.coords2[i]) this.coords2[i] = [];
                        this.coords2[i].push({
                            object: this,
                            element: rect,
                            x: parseFloat(rect.getAttribute('x')),
                ...
                //
                    ...
                    for(var j = 0; j < this.data[i].length; ++j, ++sequentialIndex){
                        var height = this.data[i][j] / (this.max - this.min) * this.graphHeight, width = section - 2 * properties.marginInner, x = properties.marginLeft + properties.marginInnerLeft + i * section + properties.marginInner, y = y - height;
                        ...
                        // Draw the errorbar if required
                        if (j === this.data[i].length - 1) this.drawErrorbar({
                            object: this,
                            element: rect,
                            index: i,
                            value: this.data[i][j],
        ...
        this.highlight = function(rect) {
            var x = rect.getAttribute('x'), y = rect.getAttribute('y'), width = rect.getAttribute('width'), height = rect.getAttribute('height');
        ...
        this.parseColors = function() {
            ...
            if (colors) for(var i = 0; i < colors.length; ++i)colors[i] = RGraph.SVG.parseColorLinear({
                object: this,
                color: colors[i]
            });
        ...
        this.grow = function() {
            var opt = arguments[0] || {
            }, frames = opt.frames || 30, frame = 0, obj = this, data = [], height = null, seq = 0;
            //
            // Copy the data
            //
            ...
            var iterate = function() {
                for(var i = 0, seq1 = 0, len = obj.coords.length; i < len; ++i, ++seq1){
                    ...
                    if (typeof data[i] === 'number') {
                        ...
                        obj.coords[seq1].element.setAttribute('height', height);
                        // Set the correct Y coord on the object
                        obj.coords[seq1].element.setAttribute('y', data[i] < 0 ? obj.getYCoord(0) : obj.getYCoord(0) - height);
                        ...
                        for(var j = 0, len2 = data[i].length; j < len2; ++j, ++seq1){
                            height = Math.abs(obj.getYCoord(data[i][j]) - obj.getYCoord(0));
                            height = multiplier * height;
                            obj.data[i][j] = data[i][j] * multiplier;
                            height = Math.round(height);
                            obj.coords[seq1].element.setAttribute('height', height);
                            obj.coords[seq1].element.setAttribute('y', data[i][j] < 0 ? obj.getYCoord(0) + accumulativeHeight : obj.getYCoord(0) - height - accumulativeHeight);
                            // This updates the size of the 3D sides to the bar
                            if (properties.variant === '3d') {
                                // Remove the 3D sides to the bar
                                if (obj.coords[seq1].element.rgraph_3d_side_face[0].parentNode) obj.coords[seq1].element.rgraph_3d_side_face[0].parentNode.removeChild(obj.coords[seq1].element.rgraph_3d_side_face[0]);
                                if (obj.coords[seq1].element.rgraph_3d_side_face[1].parentNode) obj.coords[seq1].element.rgraph_3d_side_face[1].parentNode.removeChild(obj.coords[seq1].element.rgraph_3d_side_face[1]);
                                if (obj.coords[seq1].element.rgraph_3d_top_face[0].parentNode) obj.coords[seq1].element.rgraph_3d_top_face[0].parentNode.removeChild(obj.coords[seq1].element.rgraph_3d_top_face[0]);
                                if (obj.coords[seq1].element.rgraph_3d_top_face[1].parentNode) obj.coords[seq1].element.rgraph_3d_top_face[1].parentNode.removeChild(obj.coords[seq1].element.rgraph_3d_top_face[1]);
                                // Add the 3D sides to the bar (again)
                                obj.drawSide3dFace({
                                    rect: obj.coords[seq1].element
                                });
                                // Draw the top side of the 3D bar
                                ...
                                // the bar - this is so that the front face appears
                                // above the other sides
                                if (obj.coords[seq1].element.parentNode) {
                                    var parent = obj.coords[seq1].element.parentNode;
                                    var node = parent.removeChild(obj.coords[seq1].element);
                                    parent.appendChild(node);
                                }
                            }
        ...
        this.positionTooltipStatic = function(args) {
            ...
            args.tooltip.style.left = svgXY[0] // The X coordinate of the canvas
             ...
             - tooltip.offsetWidth / 2 + coords.width / 2 + 'px';
...
//# sourceMappingURL=index.5fb5518a.js.map

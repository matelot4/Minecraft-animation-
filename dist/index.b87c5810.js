...
})({"93DBF":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4e5dac8afe405db7";
module.bundle.HMR_BUNDLE_ID = "48202d1ab87c5810";
"use strict";
...
(function(win, doc, undefined) {
    ...
    RGraph.SVG.TRIG.HALFPI = Math.PI * 0.4999;
    ...
    RGraph.SVG.createSVG = function(opt) {
        var container = opt.container, obj = opt.object;
        if (container.__svg__) return container.__svg__;
        ...
        if (style.position !== 'absolute' && style.position !== 'fixed' && style.position !== 'sticky') container.style.position = 'relative';
    ...
    RGraph.SVG.create = function(opt) {
        ...
        // Add the attributes
        for(var o in opt.attr)if (typeof o === 'string') {
            var name = o;
            if (o === 'className') name = 'class';
            if ((opt.type === 'a' || opt.type === 'image') && o === 'xlink:href') tag.setAttributeNS('http://www.w3.org/1999/xlink', o, String(opt.attr[o]));
            else {
                if (RGraph.SVG.isNull(opt.attr[o])) opt.attr[o] = '';
                tag.setAttribute(name, String(opt.attr[o]));
            }
        }
        // Add the style
        for(var o in opt.style)if (typeof o === 'string') tag.style[o] = String(opt.style[o]);
        if (opt.parent) opt.parent.appendChild(tag);
        else opt.svg.appendChild(tag);
        return tag;
    };
    //
    ...
    RGraph.SVG.drawXAxis = function(obj) {
        ...
        if (properties.xaxis) {
            ...
            // HBar X axis
            if (obj.type === 'hbar') var width = obj.graphWidth / obj.data.length, x = properties.marginLeft, startY = obj.height - properties.marginBottom, endY = obj.height - properties.marginBottom + properties.xaxisTickmarksLength;
            else {
                var width = obj.graphWidth / obj.data.length, x = properties.marginLeft, startY = obj.getYCoord(0) - (properties.yaxisScaleMin < 0 ? properties.xaxisTickmarksLength : 0), endY = obj.getYCoord(0) + properties.xaxisTickmarksLength;
            ...
            if (properties.xaxisTickmarks) {
                ...
                if (properties.xaxisScale) {
                    ...
                    for(var i = 0; i < (typeof properties.xaxisLabelsPositionEdgeTickmarksCount === 'number' ? properties.xaxisLabelsPositionEdgeTickmarksCount : obj.scale.numlabels + (properties.yaxis && properties.xaxisScaleMin === 0 ? 0 : 1)); ++i){
                        if (obj.type === 'hbar') var dataPoints = obj.data.length;
                        ...
                        if (typeof properties.xaxisLabelsPositionEdgeTickmarksCount === 'number') {
                            ...
                            // have to be adjusted
                            if (properties.yaxisPosition === 'right') x -= gap;
                        }
                    ...
                    if (properties.yaxisPosition === 'right' && properties.xaxisScaleMin < 0 && properties.xaxisScaleMax > 0) RGraph.SVG.create({
                        svg: obj.svg,
                        parent: obj.svg.all,
                        type: 'path',
                        attr: {
                    ...
                    if (properties.xaxisLabelsPosition === 'section') {
                        if (obj.type === 'bar' || obj.type === 'waterfall') var dataPoints = obj.data.length;
                        else if (obj.type === 'line') var dataPoints = obj.data[0].length;
                        ...
                        for(var i = 0; i < dataPoints; ++i){
                            ...
                            if (properties.yaxisPosition === 'right') x = properties.marginLeft + (properties.marginInnerLeft || 0) + i * ((obj.graphWidth - (properties.marginInnerLeft || 0) - (properties.marginInnerRight || 0)) / dataPoints);
                            else x = properties.marginLeft + (properties.marginInnerLeft || 0) + (i + 1) * ((obj.graphWidth - (properties.marginInnerLeft || 0) - (properties.marginInnerRight || 0)) / dataPoints);
                            RGraph.SVG.create({
                                svg: obj.svg,
                                parent: obj.svg.all,
                                type: 'path',
                        ...
                        if (properties.yaxisPosition === 'right' && !properties.yaxis) RGraph.SVG.create({
                            svg: obj.svg,
                            parent: obj.svg.all,
                            type: 'path',
                            attr: {
                    ...
                    // This style is used by line charts
                        ...
                        if (typeof properties.xaxisLabelsPositionEdgeTickmarksCount === 'number') var len = properties.xaxisLabelsPositionEdgeTickmarksCount;
                        else var len = obj.data && obj.data[0] && obj.data[0].length ? obj.data[0].length : 0;
                        for(var i = 0; i < len; ++i){
                            ...
                            if (properties.yaxisPosition === 'right') {
                                ...
                                // axis then skip it
                                if (properties.yaxis && x > obj.width - properties.marginRight - 3 && x < obj.width - properties.marginRight + 3) continue;
                            } else x = properties.marginLeft + (i + 1) * gap;
                            RGraph.SVG.create({
                                svg: obj.svg,
                                parent: obj.svg.all,
                                type: 'path',
                ...
                if (properties.yaxis === false || (properties.marginInnerLeft || 0) > 0) RGraph.SVG.create({
                    svg: obj.svg,
                    parent: obj.svg.all,
                    type: 'path',
                    attr: {
        ...
        if (properties.xaxisScale) {
            if (obj.type === 'scatter') {
                ...
                if (properties.xaxisLabelsCount > 0) {
                    var y = obj.height - properties.marginBottom + properties.xaxisLabelsOffsety + (properties.xaxis ? properties.xaxisTickmarksLength + 6 : 10), str = RGraph.SVG.numberFormat({
                        object: obj,
            ...
            // =========================================================================
                ...
                var segment = obj.graphWidth / properties.xaxisLabelsCount, scale = obj.scale;
                for(var i = 0; i < scale.labels.length; ++i){
                    var x = properties.marginLeft + segment * i + segment + properties.xaxisLabelsOffsetx;
                    var y = obj.height - properties.marginBottom + (properties.xaxis ? properties.xaxisTickmarksLength + 6 : 10) + (properties.xaxisLinewidth || 1) + properties.xaxisLabelsOffsety;
                    // If the Y axis is positioned on the RHS then the
                    // labels should be reversed (HBar)
                    if ((obj.type === 'hbar' || obj.type === 'scatter' && properties.xaxis) && properties.yaxisPosition === 'right') x = obj.width - properties.marginRight - segment * i - segment + properties.xaxisLabelsOffsetx;
                    RGraph.SVG.text({
                        object: obj,
                        parent: obj.svg.all,
        ...
        } else if (typeof properties.xaxisLabels === 'object' && !RGraph.SVG.isNull(properties.xaxisLabels)) {
            ...
            if (properties.xaxisLabelsPosition === 'section') {
                ...
                if (obj.type === 'line') var hmargin = properties.marginInner;
                else var hmargin = 0;
    ...
    RGraph.SVG.drawYAxis = function(obj) {
        ...
        if (properties.yaxis) {
            ...
            if (obj.type === 'hbar') {
                ...
                if (properties.xaxisScaleMin < 0 && properties.xaxisScaleMax <= 0) x = obj.getXCoord(properties.xaxisScaleMax);
            } else if (properties.yaxisPosition === 'right') var x = obj.width - properties.marginRight;
            else var x = properties.marginLeft;
            var axis = RGraph.SVG.create({
                svg: obj.svg,
                parent: obj.svg.all,
                type: 'path',
            ...
            //
                ...
                var height = obj.graphHeight / properties.yaxisLabelsCount, y = properties.marginTop, len = properties.yaxisLabelsCount, startX = properties.marginLeft, endX = properties.marginLeft - properties.yaxisTickmarksLength;
                ...
                if (properties.yaxisTickmarks) {
                    for(var i = 0; i < len; ++i){
                        ...
                        // Egde case
                        if (!(obj.max <= 0 && obj.min < obj.max && y === obj.properties.marginTop) && !(obj.min < 0 && obj.max > 0 && y <= obj.getYCoord(0) + 1 && y >= obj.getYCoord(0) - 1)) // Avoid a tickmark at zero                        
        ...
        if (properties.yaxisScale) {
            ...
            //
            var y = obj.height - properties.marginBottom, // Changed this to use obj.scale.min instead of properties.yaxisScaleMin
        ...
        //
        } else if (properties.yaxisLabels && properties.yaxisLabels.length) for(var i = 0; i < properties.yaxisLabels.length; ++i){
            var segment = (obj.graphHeight - (properties.marginInnerTop || 0) - (properties.marginInnerBottom || 0)) / properties.yaxisLabels.length, y = properties.marginTop + (properties.marginInnerTop || 0) + segment * i + segment / 2 + properties.yaxisLabelsOffsety, x = properties.marginLeft - 7 - (properties.yaxisLinewidth || 1) + properties.yaxisLabelsOffsetx, halign = 'right';
            if (properties.yaxisPosition === 'right') {
                halign = 'left';
    ...
    RGraph.SVG.drawBackground = function(obj) {
        ...
        if (properties.backgroundColor) RGraph.SVG.create({
            svg: obj.svg,
            parent: obj.svg.all,
            type: 'rect',
            attr: {
        ...
        if (properties.backgroundImage) {
            ...
            if (properties.backgroundImageStretch) {
                ...
                if (typeof properties.backgroundImageX === 'number') attr.x = properties.backgroundImageX + properties.variant3dOffsetx;
                else attr.x = properties.marginLeft + properties.variant3dOffsetx;
                if (typeof properties.backgroundImageY === 'number') attr.y = properties.backgroundImageY + properties.variant3dOffsety;
                else attr.y = properties.marginTop + properties.variant3dOffsety;
        ...
        if (properties.backgroundGrid) {
            ...
            if (properties.backgroundGridHlines) {
                if (typeof properties.backgroundGridHlinesCount === 'number') var count = properties.backgroundGridHlinesCount;
                else if (obj.type === 'hbar' || obj.type === 'bipolar') {
                    if (typeof properties.yaxisLabels === 'object' && !RGraph.SVG.isNull(properties.yaxisLabels) && properties.yaxisLabels.length) var count = properties.yaxisLabels.length;
                    else if (obj.type === 'hbar') var count = obj.data.length;
                    else if (obj.type === 'bipolar') var count = obj.left.length;
                } else var count = properties.yaxisLabelsCount || 5;
            ...
            if (properties.backgroundGridVlines) {
                if (obj.type === 'line' && RGraph.SVG.isArray(obj.data[0])) var len = obj.data[0].length;
                else if (obj.type === 'hbar') var len = properties.xaxisLabelsCount || 10;
                else if (obj.type === 'bipolar') var len = properties.xaxisLabelsCount || 10;
                else if (obj.type === 'scatter') var len = properties.xaxisLabels && properties.xaxisLabels.length || 10;
                else if (obj.type === 'waterfall') var len = obj.data.length;
                else var len = obj.data.length;
                var count = typeof properties.backgroundGridVlinesCount === 'number' ? properties.backgroundGridVlinesCount : len;
                if (properties.xaxisLabelsPosition === 'edge') count--;
            ...
            var dasharray;
            if (properties.backgroundGridDashed) dasharray = [
                3,
                5
            ];
            else if (properties.backgroundGridDotted) dasharray = [
                1,
                3
            ];
            else if (properties.backgroundGridDashArray) dasharray = properties.backgroundGridDashArray;
            else dasharray = '';
            // Now draw the path
        ...
        if (obj.type !== 'bipolar') RGraph.SVG.drawTitle(obj);
    };
    //
    ...
    RGraph.SVG.getScale = function(opt) {
        var obj = opt.object, properties = obj.properties, numlabels = opt.numlabels, unitsPre = opt.unitsPre, unitsPost = opt.unitsPost, max = Number(opt.max), min = Number(opt.min), strict = opt.strict, decimals = Number(opt.decimals), point = opt.point, thousand = opt.thousand, originalMax = max, round = opt.round, scale = {
            max: 1,
            labels: [],
            values: []
        ...
        //
        for(var i = 0; i < scale.values.length; ++i)scale.values[i] = parseFloat(scale.values[i]);
        return scale;
    };
    //
    ...
    RGraph.SVG.arraySum = function(arr) {
        ...
        // Account for null
        if (RGraph.SVG.isNull(arr)) return 0;
    ...
    RGraph.SVG.arrayFill = RGraph.SVG.arrayPad = function(args) {
        if (arguments.length === 1) var arr = args.array, val = args.value, len = args.length;
        else var arr = arguments[0], len = arguments[1], val = arguments[2];
        if (arr.length < len) {
            var val = typeof val !== 'undefined' ? val : null;
            for(var i = arr.length; i < len; i += 1)arr[i] = val;
        }
        return arr;
    };
    //
    ...
    RGraph.SVG.arrayLinearize = function() {
        ...
        for(var i = 0, len = args.length; i < len; ++i){
            if (typeof args[i] === 'object' && args[i]) for(var j = 0, len2 = args[i].length; j < len2; ++j){
    ...
    RGraph.SVG.arrayShift = function(arr) {
        var ret = [];
        for(var i = 1, len = arr.length; i < len; ++i)ret.push(arr[i]);
        return ret;
    };
    //
    ...
    RGraph.SVG.arrayReverse = function(arr) {
        if (!arr) return;
        var newarr = [];
        for(var i = arr.length - 1; i >= 0; i -= 1)newarr.push(arr[i]);
        return newarr;
    };
    //
    ...
    RGraph.SVG.clone = RGraph.SVG.arrayClone = function(obj) {
        ...
        if (RGraph.SVG.isArray(obj)) {
            ...
            for(var i = 0, len = obj.length; i < len; ++i){
                if (typeof obj[i] === 'number') temp[i] = (function(arg) {
                    return Number(arg);
                })(obj[i]);
                else if (typeof obj[i] === 'string') temp[i] = (function(arg) {
                    return String(arg);
                })(obj[i]);
                else if (typeof obj[i] === 'function') temp[i] = obj[i];
                else temp[i] = RGraph.SVG.arrayClone(obj[i]);
            }
    ...
    RGraph.SVG.isArray = function(obj) {
        if (obj && obj.constructor) var pos = obj.constructor.toString().indexOf('Array');
        else return false;
    ...
    RGraph.SVG.abs = function(value) {
        ...
        if (typeof value === 'object') {
            for(i in value)if (typeof i === 'string' || typeof i === 'number' || typeof i === 'object') value[i] = RGraph.SVG.abs(value[i]);
            return value;
        }
        return 0;
    };
    ...
    RGraph.SVG.numberFormat = function(opt) {
        ...
        RegExp.$1 = '';
        if (typeof opt.formatter === 'function') return opt.formatter(obj, num);
        ...
        var rev = output;
        output = '';
        for(i = rev.length - 1; i >= 0; i--)output += rev.charAt(i);
        // Tidy up
    ...
    RGraph.SVG.text = function(opt) {
        ...
        else if (halign === 'center' || halign === 'middle') halign = 'middle';
        else halign = 'start';
        // Vertical alignment
        if (valign === 'top') valign = 'hanging';
        ...
        if (link) var a = RGraph.SVG.create({
            svg: obj.svg,
            type: 'a',
            parent: parent,
            attr: {
        ...
        if (str && str.indexOf && str.indexOf("\n") === -1) {
            ...
            if (!events) text.style.pointerEvents = 'none';
        //
        ...
        //
            ...
            for(var i = 0; i < str.length; ++i){
                ...
                if (!events) tspan.style.pointerEvents = 'none';
                var dimensions = RGraph.SVG.measureText({
                    text: str[i],
                    bold: bold,
                    font: font,
        ...
        if (typeof background === 'string') {
            ...
            var bbox = text.getBBox(), rect = RGraph.SVG.create({
                svg: obj.svg,
                parent: parent,
                type: 'rect',
                attr: {
            ...
            if (!events) rect.style.pointerEvents = 'none';
            text.parentNode.insertBefore(rect, text);
        }
        if (RGraph.SVG.ISIE && valign === 'hanging' && text) text.setAttribute('y', y + text.scrollHeight / 2);
        else if (RGraph.SVG.ISIE && valign === 'middle' && text) text.setAttribute('y', y + text.scrollHeight / 3);
    ...
    RGraph.SVG.text.find = function(opt) {
        ...
        if (typeof opt.object === 'object' && opt.object.isRGraph) var svg = opt.object.svg;
        ...
        for(var i = 0, len = nodes.length; i < len; ++i){
            var text = false, tag = false;
            // Exact match or regex on the text
            if (typeof opt.text === 'string' && nodes[i].innerHTML === opt.text) text = true;
            else if (typeof opt.text === 'object' && nodes[i].innerHTML.match(opt.text)) text = true;
            else if (typeof opt.text === 'undefined') text = true;
            // Exact match or regex on the tag
            if (typeof opt.tag === 'string' && nodes[i].getAttribute('tag') === opt.tag) tag = true;
            else if (typeof opt.tag === 'object' && nodes[i].getAttribute('tag').match(opt.tag)) tag = true;
            else if (typeof opt.tag === 'undefined') tag = true;
    ...
    RGraph.SVG.hideTooltip = function() {
        ...
        if (tooltip && tooltip.__object__) RGraph.SVG.removeHighlight(tooltip.__object__);
    };
    //
    // Creates a shadow
    //
    RGraph.SVG.setShadow = function(options) {
        var obj = options.object, offsetx = options.offsetx || 0, offsety = options.offsety || 0, blur = options.blur || 0, opacity = options.opacity || 0, id = options.id;
    ...
    RGraph.SVG.groupedIndexToSequential = function(opt) {
        var dataset = opt.dataset, index = opt.index, obj = opt.object;
        for(var i = 0, seq = 0; i <= dataset; ++i)for(var j = 0; j < obj.data[dataset].length; ++j){
            if (i === dataset && j === index) return seq;
            seq++;
        }
        return seq;
    };
    ...
    RGraph.SVG.TRIG.getAngleByXY = function(opt) {
        var cx = opt.cx, cy = opt.cy, x = opt.x, y = opt.y;
        var angle = Math.atan((y - cy) / (x - cx));
        if (x >= cx && y >= cy) angle += RGraph.SVG.TRIG.HALFPI;
        else if (x >= cx && y < cy) angle = angle + RGraph.SVG.TRIG.HALFPI;
        else if (x < cx && y < cy) angle = angle + RGraph.SVG.TRIG.PI + RGraph.SVG.TRIG.HALFPI;
        else angle = angle + RGraph.SVG.TRIG.PI + RGraph.SVG.TRIG.HALFPI;
        return angle;
    };
    //
    ...
    RGraph.SVG.TRIG.getArcPath = function(options) {
        ...
        if (typeof options.moveto === 'boolean' && options.moveto === false) var d = [
            "A",
            options.r,
            options.r,
            0,
        ...
        if (options.array === true) return d;
        else return d.join(" ");
    };
    //
    // Gets a path that is usable by the SVG A path command
    ...
    RGraph.SVG.TRIG.getArcPath2 = function(options) {
        ...
        } else if (diff > RGraph.SVG.TRIG.PI) {
            largeArc = '1';
            sweep = '0';
        } else {
            largeArc = '0';
        ...
        if (typeof options.lineto === 'boolean' && options.lineto === false) var d = [
            "M",
            start.x,
            start.y,
            "A",
    ...
    RGraph.SVG.TRIG.getArcPath3 = function(options) {
        ...
        if (typeof options.radius === 'number') options.r = options.radius;
        //
        // Make circles start at the top instead of the right hand side
        //
    ...
    RGraph.SVG.TRIG.getRadiusEndPoint = function(opt) {
        ...
        if (arguments.length === 1) {
            if (typeof opt.radius === 'number') opt.r = opt.radius;
            var angle = opt.angle, r = opt.r;
        } else if (arguments.length === 4) var angle = arguments[0], r = arguments[1];
    ...
    RGraph.SVG.drawTitle = function(obj) {
        ...
        var x = (obj.width - properties.marginLeft - properties.marginRight) / 2 + properties.marginLeft, y = properties.marginTop - 10, valign = 'bottom';
        // If theres key defined then move the title up
        if (!RGraph.SVG.isNull(obj.properties.key)) y -= 20;
        ...
        // Draw the title
        if (properties.title) RGraph.SVG.text({
            object: obj,
            svg: obj.svg,
            parent: obj.svg.all,
            tag: 'title',
        ...
        if ((typeof properties.title === 'string' || typeof properties.title === 'number') && (typeof properties.titleSubtitle === 'string' || typeof properties.titleSubtitle === 'number')) RGraph.SVG.text({
            object: obj,
            svg: obj.svg,
            parent: obj.svg.all,
            tag: 'subtitle',
    ...
    RGraph.SVG.parseColorLinear = function(opt) {
        ...
        if (color.match(/^gradient\((.*)\)$/i)) {
            ...
            if (opt && opt.direction && opt.direction === 'horizontal') var grad = RGraph.SVG.create({
                type: 'linearGradient',
                parent: obj.svg.defs,
                attr: {
            ...
            else var grad = RGraph.SVG.create({
                type: 'linearGradient',
                parent: obj.svg.defs,
                attr: {
                    id: 'RGraph-linear-gradient-' + obj.uid + '-' + obj.gradientCounter,
            ...
            for(var j = 1, len = parts.length; j < len; ++j)RGraph.SVG.create({
                type: 'stop',
                parent: grad,
                attr: {
                    offset: j * diff * 100 + '%',
    ...
    RGraph.SVG.resetColorsToOriginalValues = function(opt) {
        ...
        if (obj.originalColors) {
            ...
            for(var j in obj.originalColors)if (typeof j === 'string') obj.properties[j] = RGraph.SVG.arrayClone(obj.originalColors[j]);
        }
        //
    ...
    RGraph.SVG.clear = function() {
        ...
        if (arguments.length === 0) {
            for(var i = 0; i < RGraph.SVG.OR.objects.length; i++)RGraph.SVG.clear(RGraph.SVG.OR.objects[i].svg);
            return;
        // An SVG tag has been given
        } else var svg = arguments[0];
        ...
        for(var i = 1; i <= 100; ++i){
            if (svg && svg['background' + i]) // Clear all the nodes within this group
            while(svg['background' + i].lastChild)svg['background' + i].removeChild(svg['background' + i].lastChild);
            else break;
        }
        if (svg.all) {
            ...
            if (svg.all.line_tooltip_hotspots) while(svg.all.line_tooltip_hotspots.lastChild)svg.all.line_tooltip_hotspots.removeChild(svg.all.line_tooltip_hotspots.lastChild);
        }
    };
    //
    ...
    RGraph.SVG.reset = function() {
        ...
        if (arguments.length === 0) {
            for(var i = 0; i < RGraph.SVG.OR.objects.length; i++)RGraph.SVG.reset(RGraph.SVG.OR.objects[i].svg);
            return;
    ...
    RGraph.SVG.fireCustomEvent = function(obj, name) {
        if (obj && obj.isRGraph) {
            ...
            if (typeof uid === 'string' && typeof RGraph.SVG.events === 'object' && typeof RGraph.SVG.events[uid] === 'object' && RGraph.SVG.events[uid].length > 0) {
                for(var j = 0, len = RGraph.SVG.events[uid].length; j < len; ++j)if (RGraph.SVG.events[uid][j] && RGraph.SVG.events[uid][j].event === name) RGraph.SVG.events[uid][j].func(obj);
            }
        }
    };
    //
    ...
    RGraph.SVG.removeAllCustomEventListeners = function() {
        var uid = arguments[0];
        if (uid && RGraph.SVG.events[uid]) RGraph.SVG.events[uid] = {
        };
    ...
    RGraph.SVG.removeCustomEventListener = function(obj, i) {
        if (typeof RGraph.SVG.events === 'object' && typeof RGraph.SVG.events[obj.uid] === 'object' && typeof RGraph.SVG.events[obj.uid][i] === 'object') RGraph.SVG.events[obj.uid][i] = null;
    };
    //
    ...
    RGraph.SVG.removeHighlight = function(obj) {
        ...
        if (highlight && RGraph.SVG.isArray(highlight) && highlight.length) {
            for(var i = 0, len = highlight.length; i < len; ++i)if (highlight[i].parentNode) //obj.svg.removeChild(highlight[i]);
            ...
            } else highlight.parentNode.removeChild(highlight);
        }
    };
    //
    // Removes the highlight from the chart added by tooltips (possibly others too)
    ...
    RGraph.SVG.redraw = function() {
        if (arguments.length === 1) {
            ...
            for(var i in tags)RGraph.SVG.redraw(tags[i]);
        }
    };
    //
    ...
    RGraph.SVG.parseDate = function(str) {
        ...
        for(var i = 0, len = tokens.length; i < len; ++i)if (tokens[i]) {
            // Year
            if (tokens[i].match(/^\d\d\d\d$/)) defaults.year = tokens[i];
            // Month
        ...
        // Support functions
        //
        function isMonth(str1) {
            var res = str1.toLowerCase().match(months_regex);
    ...
    RGraph.SVG.OR.get = function() {
        ...
        if (typeof arguments[0] === 'string' && arguments[0].substr(0, 3).toLowerCase() === 'id:') {
            var ret = [];
            for(var i = 0; i < RGraph.SVG.OR.objects.length; ++i)if (RGraph.SVG.OR.objects[i] && RGraph.SVG.OR.objects[i].id === arguments[0].substr(3)) ret.push(RGraph.SVG.OR.objects[i]);
            return ret;
        }
        ...
        if (typeof arguments[0] === 'string' && arguments[0].substr(0, 4).toLowerCase() === 'type') {
            var ret = [];
            for(var i = 0; i < RGraph.SVG.OR.objects.length; ++i)if (RGraph.SVG.OR.objects[i].type === arguments[0].substr(5)) ret.push(RGraph.SVG.OR.objects[i]);
            return ret;
        }
        // Fetch an object that has a specific UID
        if (typeof arguments[0] === 'string' && arguments[0].substr(0, 3).toLowerCase() === 'uid') {
            var ret = [];
            for(var i = 0; i < RGraph.SVG.OR.objects.length; ++i)if (RGraph.SVG.OR.objects[i].uid === arguments[0].substr(4)) ret.push(RGraph.SVG.OR.objects[i]);
            return ret;
        }
    ...
    RGraph.SVG.OR.clear = function() {
        ...
        if (typeof arguments[0] === 'string') {
            for(var i = 0; i < RGraph.SVG.OR.objects.length; ++i)if (RGraph.SVG.OR.objects[i].id === arguments[0]) RGraph.SVG.OR.objects[i] = null;
        // If an RGraph object is given then clear that object
        } else if (typeof arguments[0] === 'object') {
            for(var i = 0; i < RGraph.SVG.OR.objects.length; ++i)if (RGraph.SVG.OR.objects[i].uid === arguments[0].uid) RGraph.SVG.OR.objects[i] = null;
    ...
    RGraph.SVG.OR.tags = function() {
        var tags = [];
        for(var i = 0; i < RGraph.SVG.OR.objects.length; ++i)if (RGraph.SVG.OR.objects[i] && !tags[RGraph.SVG.OR.objects[i].svg.parentNode.id]) tags[RGraph.SVG.OR.objects[i].svg.parentNode.id] = RGraph.SVG.OR.objects[i].svg;
        return tags;
    };
    //
    ...
    RGraph.SVG.getSVGXY = function(svg) {
        ...
        var paddingLeft = svg.style.paddingLeft ? parseInt(svg.style.paddingLeft) : 0, paddingTop = svg.style.paddingTop ? parseInt(svg.style.paddingTop) : 0, borderLeft = svg.style.borderLeftWidth ? parseInt(svg.style.borderLeftWidth) : 0, borderTop = svg.style.borderTopWidth ? parseInt(svg.style.borderTopWidth) : 0;
    ...
    RGraph.SVG.measureText = function(opt) {
        //text, bold, font, size
        var text = opt.text || '', bold = opt.bold || false, italic = opt.italic || false, font = opt.font || 'sans-serif', size = opt.size || 12, str = text + ':' + italic + ':' + bold + ':' + font + ':' + size;
    ...
    RGraph.SVG.stringsToNumbers = function(str) {
        ...
        if (typeof str === 'string') {
            if (str.indexOf(sep) != -1) str = str.split(sep);
            else {
                str = parseFloat(str);
                if (isNaN(str)) str = null;
            }
        }
        if (typeof str === 'object' && !RGraph.SVG.isNull(str)) for(var i = 0, len = str.length; i < len; i += 1)str[i] = RGraph.SVG.stringsToNumbers(str[i], sep);
        return str;
    };
    ...
    RGraph.SVG.getAdjustedNumber = function(opt) {
        ...
        if (typeof prop === 'string' && match(/^(\+|-)([0-9.]+)/)) {
            if (RegExp.$1 === '+') value += parseFloat(RegExp.$2);
            else if (RegExp.$1 === '-') value -= parseFloat(RegExp.$2);
        }
        return value;
    };
    // NOT USED ANY MORE
    ...
    RGraph.SVG.random = function(opt) {
        var min = opt.min, max = opt.max, dp = opt.dp || opt.decimals || 0, r = Math.random();
    ...
    RGraph.SVG.arrayRandom = function(opt) {
        ...
        for(var i = 0, arr = []; i < num; i += 1)arr.push(RGraph.SVG.random({
            min: min,
            max: max,
            dp: dp
        }));
    ...
    RGraph.SVG.commonSetter = function(opt) {
        ...
        if (name === 'tooltipsEvent' && value !== 'click' && value !== 'mousemove') value = 'click';
        return {
            name: name,
            value: value
        };
    ...
    RGraph.SVG.getGlobals = function(obj) {
        var properties = obj.properties;
        for(var i in RGraph.SVG.GLOBALS)if (typeof i === 'string') obj.set(i, RGraph.SVG.arrayClone(RGraph.SVG.GLOBALS[i]));
    };
    //
    ...
    RGraph.SVG.getErrorbarsMaxValue = function(opt) {
        var obj = opt.object, properties = obj.properties, index = opt.index;
        if (typeof properties.errorbars === 'object' && !RGraph.SVG.isNull(properties.errorbars) && typeof properties.errorbars[index] === 'number') var value = properties.errorbars[index];
        else if (typeof properties.errorbars === 'object' && !RGraph.SVG.isNull(properties.errorbars) && typeof properties.errorbars[index] === 'object' && !RGraph.SVG.isNull(properties.errorbars[index]) && typeof properties.errorbars[index].max === 'number') var value = properties.errorbars[index].max;
        else var value = 0;
        return value;
    };
    ...
    RGraph.SVG.getErrorbarsMinValue = function(opt) {
        var obj = opt.object, properties = obj.properties, index = opt.index;
        if (typeof properties.errorbars === 'object' && !RGraph.SVG.isNull(properties.errorbars) && typeof properties.errorbars[index] === 'object' && !RGraph.SVG.isNull(properties.errorbars[index]) && typeof properties.errorbars[index].min === 'number') var value = properties.errorbars[index].min;
        else var value = null;
        return value;
    };
    ...
    RGraph.SVG.getErrorbarsColor = function(opt) {
        ...
        if (typeof properties.errorbars === 'object' && !RGraph.SVG.isNull(properties.errorbars) && typeof properties.errorbars[index] === 'object' && !RGraph.SVG.isNull(properties.errorbars[index]) && typeof properties.errorbars[index].color === 'string') color = properties.errorbars[index].color;
        return color;
    };
    ...
    RGraph.SVG.getErrorbarsLinewidth = function(opt) {
        ...
        if (typeof properties.errorbars === 'object' && !RGraph.SVG.isNull(properties.errorbars) && typeof properties.errorbars[index] === 'object' && !RGraph.SVG.isNull(properties.errorbars[index]) && typeof properties.errorbars[index].linewidth === 'number') linewidth = properties.errorbars[index].linewidth;
        return linewidth;
    };
    ...
    RGraph.SVG.getErrorbarsCapWidth = function(opt) {
        ...
        if (typeof properties.errorbars === 'object' && !RGraph.SVG.isNull(properties.errorbars) && typeof properties.errorbars[index] === 'object' && !RGraph.SVG.isNull(properties.errorbars[index]) && typeof properties.errorbars[index].capwidth === 'number') capwidth = properties.errorbars[index].capwidth;
        return capwidth;
    };
    ...
    RGraph.SVG.responsive = function(conf) {
        ...
        for(var i = 0; i < conf.length; ++i){
            if (conf[i + 1] && typeof conf[i + 1].maxWidth === 'number') conf[i].minWidth = conf[i + 1].maxWidth;
            else if (!conf[i + 1]) conf[i].minWidth = 0;
        }
        //
        ...
        for(var i = 0; i < conf.length; ++i){
            ...
            // An Initial test
            if (document.documentElement.clientWidth >= conf[i].minWidth && document.documentElement.clientWidth < conf[i].maxWidth) matchFunction(conf[i]);
        }
        //
        ...
        function matchFunction(rule) {
            ...
            if (typeof rule.options === 'object') {
                for(var j in rule.options)if (typeof j === 'string') obj.set(j, rule.options[j]);
            }
            //
            ...
            if (typeof rule.css === 'object') {
                for(var j in rule.css)if (typeof j === 'string') setCSS(obj.svg.parentNode, j, rule.css[j]);
            }
            //
            // Are there any CSS properties to set on the SVGs PARENT tag?
            //
            if (typeof rule.parentCss === 'object') {
                for(var j in rule.parentCss)if (typeof j === 'string') setCSS(obj.svg.parentNode.parentNode, j, rule.parentCss[j]);
            }
        ...
        var func = function() {
            ...
            if (!matched && (document.documentElement.clientWidth <= conf[i].maxWidth || RGraph.SVG.isNull(conf[i].maxWidth))) {
                ...
                if (typeof conf[i].options === 'object' && typeof conf[i].options === 'object') {
                    for(var j in conf[i].options)if (typeof j === 'string') obj.set(j, conf[i].options[j]);
                }
                //
                // This function simply sets a CSS property on the object.
                // It accommodates certain name changes
                ...
                if (typeof conf[i].css === 'object') {
                    for(var j in conf[i].css)if (typeof j === 'string') setCSS(obj.svg.parentNode, j, conf[i].css[j]);
                }
                //
                // Are there any CSS properties to set on the SVGs PARENT tag?
                //
                if (typeof conf[i].parentCss === 'object') {
                    for(var j in conf[i].parentCss)if (typeof j === 'string') setCSS(obj.svg.parentNode.parentNode, j, conf[i].parentCss[j]);
                }
                // Redraw the chart with SVG this is done by the redraw() function
                RGraph.SVG.redraw();
                // Run the callback function if it's defined
                if (typeof conf[i].callback === 'function') conf[i].callback(obj);
            }
        };
...
window.$p = function(obj) {
    ...
    switch(typeof obj){
        ...
        case 'object':
            // In case of null
            if (RGraph.SVG.isNull(obj)) str += indent + 'null\n';
            else {
                str += indent + 'Object {' + '\n';
...
//# sourceMappingURL=index.b87c5810.js.map
